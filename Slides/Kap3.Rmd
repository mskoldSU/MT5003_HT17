...
men saknar såväl $\theta_0$ som användbart uttryck för $h(\theta)$.

## Bootstrap (Plug-in + Monte Carlo)

- **Plug-in:** Skatta $P_{\theta_0}$ med $P_{\hat{\theta}}$.
- **Monte Carlo:** Approximera $h(\hat{\theta})$ genom att
simulera $N$ nya stickprov $x_1^*,\ldots, x_N^*$, $x_i^*\in\mathcal{F}$, från $P_{\hat{\theta}}$ och använd dessa för att empiriskt approximera storheten av intresse.
...
# Bestämmer ML-skattning för varje stickprov och spara i theta.boot
theta.hat <- 1 / mean(ac)
N <- 10000
...
```{r, echo = TRUE}
# Drar N stickprov av storlek 28 från empirisk fördelning
# Bestämmer skattning för varje stickprov och approximera standardfel
N <- 10000
...
<div class="boxed"> **Definition:** En test-variabel $T$ är en stickprovsvariabel som mäter avvikelsen från en hypotes $H$. Det (ensidiga) $P$-värdet definieras som $P_{\theta_0}(T>t)$ för en enkel hypotes och  $\sup_{\{\theta\in\Theta_0\}} P_{\theta}(T>t)$ för en sammansatt.
</div>
...
<div class="boxed"> **Hjälpsats:** Om $X$ har fördelningsfunktion $F$ så är $F(X)$ (och då även $1-F(X)$) likformigt fördelade på $[0,1]$.
</div>
