...
"Basic Bootstrap"-intervallet bygger på att $g(x,\theta)=\hat{\theta}(x)-\theta$ är en approximativ pivå, d.v.s. dess fördelning är nästan oberoende av $\theta$ (åtminstone i en omgivning av det sanna värdet). Då kan vi använda Monte-Carlo för att approximera $a$ och $b$ så att $P_\hat{\theta}(a<\hat{\theta}(X)-\hat{\theta}(x)<b)=\gamma$, varur ett konfidensintervall löses ut som $(\hat{\theta}-b, \hat{\theta}-a)$. Utöver Monte-Carlo approximationen i bestämning av $a$ och $b$ gör vi även ett skattningsfel då $P_{\theta_0}(a<\hat{\theta}(X)-{\theta_0}<b)$ kan skilja sig något från $\gamma$ då $\hat{\theta}(x)-\theta$ inte är en exakt pivåvariabel.
