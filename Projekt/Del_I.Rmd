...
$$
där $y=(y_1,\ldots,y_N)^T$ är vår realisering av $Y$, $X$ en $N\times k$-matris med $x_i$ på rad $i$, $p=(p_1,\ldots,p_N)$ och slutligen $D$ en diagonalmatris med diagonal $v=(v_1,\ldots,v_N)^T$ där $v_i=p_i(1-p_i)$, $i=1, \ldots, n$.
...
Uppskatta standardavvikelserna hos ML-skattarna av $\theta$ med parametrisk Bootstrap (utan att använda funktionalitet från paketet `boot`) och jämför med de som ges i föregående uppgift. Bilda även ett Bootstrap-baserat 95% konfidensintervall för sannolikheten att en privatist med din egen ålder och kön klarar en uppkörning.
